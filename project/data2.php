<?	//  [ 변수처리 ] =========== 
	...
	if($mcode) {
		...
		from (
            ...
            where mcode like '{$mcode}%' and date like '{$s_date}%' and type like '{$type}%' ) as a
		group by a.n_time";
	    ...
	    echo "<div class='head'>선택한 지면 : ".$mcode."의 시간대 별 ".$type." 카운트 입니다.</div>";
        echo "<h3>일자: {$s_date} / 지면: {$mcode} / 상태: {$type}</h3>";
        for($i=0;$i<=23;$i++){
            $num = sprintf('%02d',$i);
